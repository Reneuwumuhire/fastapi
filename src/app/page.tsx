...
export default function Home() {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			const response = await fetch(`https://fastapi.fastapirw.workers.dev/api/search?q=${input}`);
	...
	return (
		<main className=" h-screen w-screen grainy">
			<div className=" flex flex-col gap-6 items-center pt-32 duration-500 animate-in animate fade-in-5 slide-in-from-bottom-2.5">
				<h1 className=" text-5xl tracking-tight font-bold text-zinc-900">⚡ SuperFast API</h1>
				...
				<div className=" max-w-md w-full">
					<Command>
						<CommandInput value={input} onValueChange={setInput} placeholder="Seach contries..." className=" placeholder:text-zinc-500" />
						<CommandList>
